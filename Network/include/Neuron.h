...
public:
    ...
    void UpdateLastGradientIfHidden(const std::vector<Neuron>& downStreamLayer);
    void UpdateInputWeights(const std::vector<Neuron>& downStreamLayer);
    float GetInputWeight(size_t i) const
    {
        return m_inputWeight[i];
    }
    float GetLastOutput() const
    {
        return m_lastOutput;
    }
    float GetLastGradient() const
    {
        return m_lastGradient;
    }
    void SetLastOutput(float value)
    {
        m_lastOutput = value;
    }
    void SetIsBias(bool value)
    {
        m_isBias = value;
    }
    bool IsBias()
